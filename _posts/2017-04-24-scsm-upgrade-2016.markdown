...
As you will be possibly maintainig 2 versions of your code from here on, it is advised to minimize the effort you put in 2012 afterwards (unless you are a vendor fo course). During the upgrade project phase, MP code changes should be avoided to prevent regression issues.
...
> NOTE: it is advised to clear every user's SCSM console to minimize possible issues
...
* If it still seems that new versions are not getting activated in the console, try to restart the 'System Center Data Access Service' as it sometimes seems to cache some stuff server-side.
...
* In all cases, as the last step, you shoud apply the latest update rollup
...
Validation is key post-upgrade. Both you and all SCSM stakeholders should validate their processes and subit feedback if anything is out of the ordinary. Foreseeing some days of after-care is recommended. Also, don't forget to upgrade your consoles ;)
